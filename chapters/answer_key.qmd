...
### 3-3. 正則化 (Regularization)
問1. C  

::: {.callout-note title="解説"}
(B)は「正則化項」の説明。正則化項にノルムを用いることはあるが、両者は同一ではない。  
$L^p$正則化は重みの$L^p$ノルムをp乗してハイパーパラメータ$Λ$を掛けたものを正則化項として損失関数に加える。
:::

問2. B  

### 3-4. 畳み込みニューラルネットワーク (Convolutional Neural)
問1-I. B  
問1-II. B  
問2. D  

::: {.callout-note title="解説"}
各畳み込み層における受容野 (Receptive Field)の広がりは、次の式で表されます。
...
- $R_{L}$ ： L層目までの受容野サイズ  
- $k_{L}$ ： L層目のカーネルサイズ  
- $S_{L-1}$ ： それまでのストライドの積（ストライド=1なら常に1）  

初期条件：$R_{0} = 1$

これにより、３X３の２層の畳み込みと５X５の１層の畳み込みは受容野が同じになる。

また、(A)について、5×5が常に「より抽象的」とは限らないので注意。
むしろ 3×3 を2層重ねる方が段階的に特徴を組み合わせられるため、抽象度が高まることが多い。  
...
::: {.callout-note title="解説"}
TransformerのAttentionの確率分布は以下のようにして求められる。  

STEP1. 内積$𝑄K^T$を求める。  
STEP2. $d_k=3$を利用し、スケーリングする。  
$$
\frac{QK^\top}{\sqrt{d_k}} = \frac{[1, 1, 2]}{\sqrt{3}} = \left[\frac{1}{\sqrt{3}}, \frac{1}{\sqrt{3}}, \frac{2}{\sqrt{3}}\right]
$$
STEP3. ソフトマックスにかける。
:::
...
### 4-6. 生成モデル (Generative Model)
問1. C  
問2-I. Deep Convolutional Generative Adversarial Network  
問2-II. B  

### 4-7. 深層強化学習 (Reinforcement Learning)
問1. I: 4 II: 4  
問2. D  
問3. A  

### 4-8. 様々な学習方法 (Various Learning Methods)
問1. A  
問2. A  

### 4-9. 深層学習の説明性 (Explainability)
問1. B  
問2. A  
問3. B  
問4. B,C  
...
::: {.callout-note title="解説"}
ResNet50やGoogLeNetでは出力する際に全結合を行わず、Global Average Poolingを行う。
これは出力前の特徴マップのそれぞれのチャネルが特定クラスの特徴を示しているということを仮定しているため。
:::
